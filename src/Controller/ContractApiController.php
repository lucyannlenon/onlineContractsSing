...
class ContractApiController extends AbstractController
{
    ...
    public function index(#[MapRequestPayload] CreateContractDto $contractDto, CreateContractService $contractService, Request $request ): Response
    {
    ...
    public function load(Contracts $contract, Request $request ): Response
    {
        $token = $request->query->get('token', null);
        if($token != $_ENV['API_TOKEN'] || $token==null){
            return  $this->json([],403);
